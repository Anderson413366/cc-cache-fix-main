...
$VersionFile = Join-Path $Base "SUPPORTED_CLAUDE_CODE_VERSION"
if (-not (Test-Path -Path $VersionFile)) {
    throw "Missing supported version file: $VersionFile"
}
$SupportedVersion = (Get-Content -Path $VersionFile -Raw).Trim()
$Version = if ($env:CC_CACHE_FIX_CLAUDE_VERSION) { $env:CC_CACHE_FIX_CLAUDE_VERSION } else { $SupportedVersion }
...
if (-not (Test-Path -Path $CliPath)) {
    ...
    if (($Version -eq $SupportedVersion) -and (Test-Path -Path (Join-Path $NodeDir "package-lock.json"))) {
        npm ci --prefix $NodeDir --ignore-scripts | Out-Host
    } else {
        npm install --prefix $NodeDir --no-save --ignore-scripts "@anthropic-ai/claude-code@$Version" | Out-Host
    }
} else {
