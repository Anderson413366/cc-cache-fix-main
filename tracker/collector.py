...
# Sessions without a marker are treated as stock so the dashboard can compare
# ordinary Claude Code runs against patched runs.
...
class Collector:
    ...
    def __init__(self, db_path: str | Path | None = None) -> None:
        ...
        # session_id -> detected mode ('stock' | 'patched' | 'unknown')
    ...
    def _process_line(self, raw_line: str) -> None:
        ...
        if session_id not in self._sessions_seen:
            ...
            if final_mode in {"stock", "patched"}:
                upsert_session(self.conn, session_id, mode=final_mode)
